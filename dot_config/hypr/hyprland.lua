...
hl.bind(mainMod .. " + T", hl.dsp.exec_cmd("$BROWSER --new-window https://chatgpt.com"))
...
function bindWorkspace(key, wsNum)
	hl.bind(mainMod .. " + " .. key, hl.dsp.focus({ workspace = wsNum }))
	hl.bind(mainMod .. " + SHIFT + " .. key, hl.dsp.window.move({ workspace = wsNum, follow = false }))
end

for i = 1, 10 do
	bindWorkspace(i % 10, i + 10)
end
bindWorkspace("grave", 1)
...
hl.window_rule({
	name = "special-border-colors",
	match = { workspace = "special:magic" },

	border_color = { colors = { "rgb(f5c2e7)", "rgb(f9e2af)" }, angle = 45 },
})
